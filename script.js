...
function buildAlphaNamesIRB(names){
  ...
  function encodeEditableTIFF(pixels5ch, width, height){
    ...
    const stripOffset = offset; offset += pixels5ch.length;
    ...
    entry(273,4,1,stripOffset,false);    // StripOffsets
    entry(277,3,1,channels,false);       // SamplesPerPixel
    entry(278,4,1,height,false);         // RowsPerStrip (single strip)
    entry(279,4,1,pixels5ch.length,false); // StripByteCounts
